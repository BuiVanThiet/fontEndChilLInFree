...
export const validateSelect = (value) => {
    if (value === null) {
        return "chưa được chọn!"; // ❌ Không phải số
    }
    return true;
}
...
// Kiểm tra chuỗi nhập
export const validateString = (value, quantityLengt) => {
    if (typeof value !== "string" || value.trim() === "") {
        return "phải có ít nhất 1 ký tự!"; // ❌ Rỗng hoặc không phải chuỗi
    }

    if (value.length > quantityLengt) {
        return "không được vượt quá " + quantityLengt + " ký tự!"; // ❌ Quá dài
    }
...
//     console.log('truyen vao: ', value, arr, action, nameColumn, valueDelete)
...
export const checkDuplicate = (value, arr, action, nameColumn, valueDelete) => {
    console.log('Truyền vào:', { value, arr, action, nameColumn, valueDelete });

    if (!Array.isArray(arr)) {
        return "Danh sách kiểm tra phải là một mảng";
    }

    if (action === 1) { // Kiểm tra trùng
        return arr.some(item => item.object2 === value) ? "đã tồn tại" : true;
    }

    if (action === 2) { // Xóa phần tử theo cột và giá trị
        console.log('Dữ liệu trong mảng:', JSON.stringify(arr, null, 2));
        console.log("Tìm kiếm với:", nameColumn, "=", valueDelete, "(", typeof valueDelete, ")");

        const index = arr.findIndex(item => String(item[nameColumn]) == valueDelete);
        console.log("Index tìm thấy:", index);
        ...
        return "Không tìm thấy giá trị cần xóa";
    }
};
