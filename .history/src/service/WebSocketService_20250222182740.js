...
class WebSocketService {
    constructor() {
        this.client = null;
        this.isConnected = false;
        this.subscribers = []; // Lưu danh sách subscribe chờ kết nối
    }
    ...
    connect(callback) {
        ...
        // Thêm token vào URL thay vì Header (do SockJS không hỗ trợ headers)
        ...
        this.client = new Client({
            ...
            onConnect: () => {
                ...
                if (callback) callback();
                this.resubscribePending(); // Đăng ký lại các subscribe bị pending
            },
            onStompError: (frame) => {
                console.error("❌ Lỗi STOMP:", frame);
            },
    ...
    subscribeSocket(callback, typeSocket) {
        if (!this.client || !this.client.connected) {
            console.warn("WebSocket chưa kết nối. Đợi kết nối rồi mới subscribe...");
            this.subscribers.push(() => this.subscribeSocket(callback, typeSocket));
            return;
        }
    ...
    subscribeToProductUpdates(callback) {
        this.subscribeSocket(callback, "/topic/product");
    }
}
