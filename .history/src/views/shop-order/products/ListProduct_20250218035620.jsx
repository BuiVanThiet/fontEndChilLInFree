...
//                         {
//                             convertData(this.props.products)
//                                 .filter(item => item.object15 > 0) // Lọc chỉ những item có object14 > 0
//                                 .map((item) => (
//                                     <Product
//                                         key={item.object1} // Thêm key để tối ưu React rendering
...
import WebSocketService from '../../../service/WebSocketService';
import { fetchProduct } from '../../../store/action/ProductAction';
import { convertData } from '../../../store/action/ActionRenderData';

class ListProduct extends React.Component {
    state = {
        currentPage: 1,
        pageSize: 2, // Số sản phẩm mỗi trang
    };
    ...
    render() {
        ...
        // Lọc và cắt dữ liệu theo trang
        const filteredProducts = convertData(products).filter(item => item.object15 > 0);
