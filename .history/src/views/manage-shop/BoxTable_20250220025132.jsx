...
const BoxTable = ({ columnsColor, dataColor, checkBoxTable, rowSelection, dataRootProductDetail }) => {
  ...
  if (dataRootProductDetail) {
    const isDuplicate = (record) => {
      console.log('check Trung:', dataRootProductDetail)
      return dataRootProductDetail.some(root => record.idColor === root.object5 && record.idSize === root.object2);
    };
  }
  ...
  return (
    <Table
      rowClassName={(record) =>
        dataRootProductDetail !== null && isDuplicate(record) ? "row-duplicate" : ""
      }
