...
class PageColor extends React.Component {
    ...
    render() {
                            ...
                            <br />
                            {
                                this.state.attribute?.object1 === attribute.object1 ?
                                    <span className="text-red-600">
                                        {validateString(this.state.attribute.object2) === true ? '' : '*Mã thuộc tính ' + validateString(this.state.attribute.object2)}
                                        {checkDuplicate(this.state.attribute.object2, this.state.listCheckSame, 1) === true ? '' : '*Mã thuộc tính ' + checkDuplicate(this.state.attribute.object2, this.state.listCheckSame, 1)}
                                    </span>
                                    : ''
                            }
                        ...
                        this.state.attribute?.object1 === attribute.object1 ?
                            <>
                                ...
                                <br />
                                {
                                    this.state.attribute?.object1 === attribute.object1 ?
                                        <span className="text-red-600">
                                            {validateString(this.state.attribute.object2) === true ? '' : '*Tên thuộc tính ' + validateString(this.state.attribute.object2)}
                                            {checkDuplicate(this.state.attribute.object2, this.state.listCheckSame, 1) === true ? '' : '*Mã thuộc tính ' + checkDuplicate(this.state.attribute.object2, this.state.listCheckSame, 1)}
                                        </span>
                                        : ''
                                }
                    ...
                    this.state.attribute?.object1 === attribute.object1 ?
                        <>
                            ...
                            <br />
                            {
                                this.state.attribute?.object1 === attribute.object1 ?
                                    <span className="text-red-600">
                                        {validateString(this.state.attribute.object3) === true ? '' : '*Tên thuộc tính ' + validateString(this.state.attribute.object3)}
                                    </span>
                                    : ''
                            }
                        </>
        ...
        let listAtribute;
        if (this.props.atTribute_type === 1) {
            listAtribute = this.props.colors.length < 0 ? null : convertData(this.props.colors);
        } else if (this.props.atTribute_type === 2) {
            listAtribute = this.props.categores.length < 0 ? null : convertData(this.props.categores);
        } else if (this.props.atTribute_type === 3) {
            listAtribute = this.props.manufacturers.length < 0 ? null : convertData(this.props.manufacturers);
        } else if (this.props.atTribute_type === 4) {
            listAtribute = this.props.origins.length < 0 ? null : convertData(this.props.origins);
        } else if (this.props.atTribute_type === 5) {
            listAtribute = this.props.sizes.length < 0 ? null : convertData(this.props.sizes);
        } else if (this.props.atTribute_type === 6) {
            listAtribute = this.props.weightTypes.length < 0 ? null : convertData(this.props.weightTypes);
        } else {
