        ...
        const columns = [
            ...
            },
            {
                ...
                render: (_, data) => (
                    <div className="flex items-center justify-start space-x-2">
                        {this.state.selectedRowKeysProductDetail.includes(data.object1) ? (
                            <Input value={formatNumberWithCommasText(this.state.dataChangeUpdate.filter(item => item.object1 === data.object1).map(item => item.object12))} onChange={(e) => this.changeMultyData(e.target.value, 'object14', data.object1)} />
                        ) : (
