...
class PageAddProductDetail extends React.Component {
    ...
    updateVariants = () => {
        this.setState(prevState => {
            const { selectedColors, selectedSizes } = prevState;
            const existingVariants = Array.isArray(prevState.productVariations) ? [...prevState.productVariations] : [];
            ...
            selectedColors.forEach(color => {
                selectedSizes.forEach(size => {
                    ...
                    const isExist = existingVariants.some(variant => variant.color === color && variant.size === size);
                    if (!isExist) {
                        newVariants.push({
                            key: key++,
                            color,
