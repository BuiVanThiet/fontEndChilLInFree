...
class PageAddProductDetail extends React.Component {
    ...
    updateVariants = () => {
        this.setState(prevState => {
            ...
            // Gộp danh sách cũ + mới, sắp xếp theo color.value và loại bỏ phần tử không hợp lệ
            const sortedVariants = [...existingVariants, ...newVariants]
                .filter(variant => variant.color?.value && variant.size?.value) // Loại bỏ nếu thiếu color.value hoặc size.value
                .sort((a, b) => {
