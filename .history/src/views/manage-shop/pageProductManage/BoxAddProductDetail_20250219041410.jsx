...
class PageAddProductDetail extends React.Component {
    ...
    changeMultyData = (value, objectChange, key) => {
        ...
        this.setState({ productVariations: updatedVariations }, () => this.validateAddProductDetail());
    };

    validateAddProductDetail = () => {
        const { productVariations } = this.state;
        productVariations.forEach(prd => {
            if(validateInteger(removeCommas(prd.quantity) !== true || validateInteger(removeCommas(prd.importPrice)) || validateInteger(removeCommas(prd.quantity) )
        })
