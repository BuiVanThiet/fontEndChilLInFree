...
class PageUpdateProduct extends React.Component {
    ...
    async componentDidUpdate(prevProps) {
        ...
        if (prevProps.params.id !== id && id) {
            ...
            // Hủy WebSocket cũ trước khi lắng nghe dữ liệu mới
            ...
            // Đăng ký lại WebSocket cho ID mới
            WebSocketService.subscribeSocket(() => {
                this.props.getProduct(id);
                this.props.getListImage(id);
            }, '/topic/product');
    ...
    unsubscribeFromWebSocket = () => {
        WebSocketService.subscribeSocket('/topic/product');
        WebSocketService.subscribeSocket('/topic/color');
        WebSocketService.subscribeSocket('/topic/category');
        WebSocketService.subscribeSocket('/topic/manufacturer');
        WebSocketService.subscribeSocket('/topic/origin');
        WebSocketService.subscribeSocket('/topic/size');
        WebSocketService.subscribeSocket('/topic/weight-type');
    };
