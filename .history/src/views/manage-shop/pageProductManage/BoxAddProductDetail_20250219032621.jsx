...
class PageAddProductDetail extends React.Component {
    ...
    updateVariants = () => {
        this.setState(prevState => {
            ...
            // Gộp danh sách cũ + mới và sắp xếp theo color.value (đảm bảo luôn là string)
            const sortedVariants = [...existingVariants, ...newVariants].sort((a, b) => {
                const colorA = a.color?.value ? String(a.color.value) : "";
                const colorB = b.color?.value ? String(b.color.value) : "";
    ...
    onClickDeleteProductDetail = (productDetail) => {
        ...
        }, () => { this.checkColorsAndSizes() this.setState({ checkDataAddProductDetail: this.state.productVariations.length > 0 ? true : false }) })
    }
