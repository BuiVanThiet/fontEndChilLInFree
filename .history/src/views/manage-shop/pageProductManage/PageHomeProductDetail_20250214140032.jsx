...
class PageHomeProductDetail extends React.Component {
    ...
    loadDataAttribute = (id) => {
        ...
        if (!this.props.weightTypes.length || idCheck !== id) {
            idCheck = id;
            this.props.fetchProductDetail(id);
        }
    }
    ...
    onSelectChange = (selectedRowKeysProductDetail) => {
        console.log('selectedRowKeys changed: ', selectedRowKeysProductDetail);
        const selectedObject1 = selectedRowKeysProductDetail.map(row => row.object1);
        this.setState({ selectedRowKeysProductDetail: selectedObject1 })
    };
