...
class PageProduct extends React.Component {
    ...
    componentDidMount() {
        WebSocketService.connect(() => {
            WebSocketService.subscribeSocket(() => this.props.fetchProduct('HAVING COUNT(prd.id_product) > 0;'), '/topic/product'); // Sử dụng arrow function
    ...
    loadDataAttribute = () => {
        ...
        if (!this.props.products.length) {
            this.props.fetchProduct('HAVING COUNT(prd.id_product) > 0;');
        }
    }
...
const mapDispatchToProps = (dispatch) => ({
    fetchProduct: (check) => dispatch(fetchProduct(check)),
