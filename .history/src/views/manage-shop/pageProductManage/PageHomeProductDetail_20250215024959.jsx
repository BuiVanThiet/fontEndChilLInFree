        ...
        const columns = [
            ...
            },
            {
                ...
                render: (_, data) => (
                    <div className="flex items-center justify-start space-x-2">
                        {this.state.selectedRowKeysProductDetail.includes(data.object1) ? (
                            <Input value={formatNumberWithCommasText(this.state.dataChangeUpdate.filter(item => item.object1 === data.object1).map(item => item.object13))} onChange={(e) => this.changeMultyData(formatNumberWithCommas(e.target.value), 'object11', data.object1)} />
                        ) : (
