...
class PageAddProductDetail extends React.Component {
    ...
    updateVariants = () => {
        ...
        const newVariants = Array.isArray(productVariations) ? [...productVariations] : []; // ✅ Đảm bảo là mảng

        let key = newVariants.length > 0 ? Math.max(...newVariants.map(v => v.key)) + 1 : 1;

        selectedColors.forEach(color => {
            selectedSizes.forEach(size => {
                const isExist = newVariants.some(variant => variant.color === color && variant.size === size);
                if (!isExist) {
                    newVariants.push({
                        key: key++,
                        color,
        ...
        this.setState({ productVariations: newVariants });
    };
