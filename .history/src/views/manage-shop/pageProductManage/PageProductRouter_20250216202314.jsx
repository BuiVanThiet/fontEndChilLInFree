...
import { validateInteger, validateNumber, validateString, checkDuplicate } from '../../../store/action/ActionRenderData';
...
class PageProductRouter extends React.Component {
    ...
    validateProduct = (code, name, status, idCategory, idManufacturer, idOrigin, describe, listImage) => {
        ...
        if (checkLenght === true && (status === 1 || status === 2) && validateInteger(idCategory) && validateInteger(idManufacturer) && validateInteger(idOrigin) && listImage.length > 0) {
            return true;
        } else {
            return false;
        }
