...
class PageAddProductDetail extends React.Component {
    ...
    updateVariants = () => {
        this.setState(prevState => {
            ...
            // Gộp danh sách cũ + mới và lọc các trường hợp không hợp lệ
            const mergedVariants = [...existingVariants, ...newVariants];

            // Loại bỏ nếu có cùng size.value nhưng khác color.value, hoặc cùng color.value nhưng khác size.value
            const filteredVariants = mergedVariants.filter((variant, _, array) => {
                return !array.some(otherVariant =>
                    (variant.size?.value === otherVariant.size?.value && variant.color?.value !== otherVariant.color?.value) ||
                    (variant.color?.value === otherVariant.color?.value && variant.size?.value !== otherVariant.size?.value)
                );
            });

            // Sắp xếp theo color.value
