                ...
                render: (_, data) => (
                    <div className="flex items-center justify-start space-x-2">
                        {this.state.selectedRowKeysProductDetail.includes(data.object1) ? (
                            <Input value={formatNumberWithCommasText(this.state.dataChangeUpdate.filter(item => selectedRowKeys.includes(item.key)))} onChange={(e) => this.changeMultyData(formatNumberWithCommas(e.target.value), 'object14', data.object1)} />
                        ) : (
