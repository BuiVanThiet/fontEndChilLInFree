...
class PageAddProductDetail extends React.Component {
    ...
    updateVariants = () => {
        ...
        const variants = Array.isArray(productVariations) ? [...productVariations] : []; // ✅ Đảm bảo là mảng

        let key = variants.length > 0 ? Math.max(...variants.map(v => v.key)) + 1 : 1;

        selectedColors.forEach(color => {
            selectedSizes.forEach(size => {
                // Kiểm tra xem biến thể này đã tồn tại chưa
                const isExist = variants.some(variant => variant.color === color && variant.size === size);
                if (!isExist) {
                    variants.push({
                        key: key++,
                        color,
