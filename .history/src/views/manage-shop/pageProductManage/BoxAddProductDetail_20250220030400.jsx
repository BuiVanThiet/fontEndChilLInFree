...
class PageAddProductDetail extends React.Component {
    ...
    saveProductDetail = async () => {
        ...
        let checkSameProductDetail = true;
        if (id) {
            ...
            newData.forEach(add => {
                convertData(this.props.listProductDetail).forEach(root => {
                    if (add.idColor === root.object5 && add.idSize === root.object2) {
                        console.log('co trung lap: ', add)
                        checkSameProductDetail = false;
                    }
                })
            })
            console.log("ko trung lap")
        }
        if (checkSameProductDetail === true) {
        ...
        if (id) {
            if (checkSameProductDetail === true) {
                ...
                toast.error('Thêm mới thất bại do có biến thể đã tồn tại!', {
                    position: "top-right",
                    autoClose: 5000,
                    hideProgressBar: false,
                    closeOnClick: true,
        ...
        return (
            <>
                <>
                    ...
                    <BoxTable columnsColor={columns} dataColor={listData} checkBoxTable={1} rowSelection={rowSelection} dataRootProductDetail={convertData(this.props.listProductDetail)} />
                    ...
                    {id && <div className='text-red-600 mb-3 flex items-end justify-end'>*Lưu ý: Hàng có màu đỏ là biến thể của sản phẩm đó đã tồn tại!</div>}
                </>
