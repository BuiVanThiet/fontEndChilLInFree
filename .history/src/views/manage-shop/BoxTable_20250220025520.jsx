...
const BoxTable = ({ columnsColor, dataColor, checkBoxTable, rowSelection, dataRootProductDetail }) => {
  ...
  const isDuplicate = (record) =>
    dataRootProductDetail?.some(root => record.color.value === root.object5 && record.idSize === root.object2) ?? false;

  console.log('check datta:', isDuplicate)

  return (
    <Table
      rowClassName={(record) => {
        const isDup = dataRootProductDetail !== null && isDuplicate(record);
        console.log("Record:", record, "Is Duplicate:", isDup); // Kiểm tra từng dòng
